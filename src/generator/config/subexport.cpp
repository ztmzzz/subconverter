...
void
proxyToClash(std::vector<Proxy> &nodes, YAML::Node &yamlnode, const ProxyGroupConfigs &extra_proxy_group, hash_t target,
             extra_settings &ext) {
    ...
    for (Proxy &x: nodes) {
        singleproxy.reset();
        if ((x.Type == ProxyType::Hysteria || x.Type == ProxyType::VLESS) && target != "clashmeta"_hash) {
            continue;
        }
        ...
        switch (x.Type) {
            case ProxyType::Shadowsocks:
                ...
                if (!tfo.is_undef() && target == "clashmeta"_hash)
...
std::string proxyToClash(std::vector<Proxy> &nodes, const std::string &base_conf,
                         ...
                         hash_t target, extra_settings &ext) {
    YAML::Node yamlnode;
    ...
    proxyToClash(nodes, yamlnode, extra_proxy_group, target, ext);
