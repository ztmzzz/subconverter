    ...
    for (uint32_t i = 0; i < yamlnode[section].size(); i++) {
        ...
        switch (hash_(proxytype)) {
            ...
            case "vless"_hash:
                ...
                singleproxy["uuid"] >>= id;
                aid = singleproxy["aid"].IsDefined() ? safe_as<std::string>(singleproxy["aid"]) : "0";
                cipher = singleproxy["cipher"].IsDefined() ? safe_as<std::string>(singleproxy["cipher"]) : "auto";
                net = singleproxy["network"].IsDefined() ? safe_as<std::string>(singleproxy["network"]) : "tcp";
                flow = singleproxy["flow"].IsDefined() ? safe_as<std::string>(singleproxy["flow"]) : "";
                fp = singleproxy["client-fingerprint"].IsDefined() ? safe_as<std::string>(
                ...
                switch (hash_(net)) {
                    case "tcp"_hash:
                        host = singleproxy["servername"].IsDefined() ? safe_as<std::string>(singleproxy["servername"])
                                                                     : "";
                        edge.clear();
                        break;
                    case "grpc"_hash:
                ...
                tls = safe_as<std::string>(singleproxy["tls"]) == "true" ? "tls" : "";
                udp = safe_as<std::string>(singleproxy["udp"]) == "true";
                scv = safe_as<std::string>(singleproxy["skip-cert-verify"]) == "true";
                vlessConstruct(node, group, ps, server, port, type, id, aid, net, cipher, flow, mode, path, host, edge,
                               tls, pbk, sid, fp, udp, tfo, scv);
                break;
            case "ss"_hash:
                group = SS_DEFAULT_GROUP;
